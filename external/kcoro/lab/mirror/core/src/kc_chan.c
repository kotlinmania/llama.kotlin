...
static inline struct kc_pending_send *pending_send_pop(struct kc_chan *ch)
{
...
static inline void pending_recv_append(struct kc_chan *ch, struct kc_pending_recv *node)
{
    node->next = NULL;
...
static inline struct kc_pending_recv *pending_recv_pop(struct kc_chan *ch)
{
...
void kc_chan_close(kc_chan_t *c)
{
    ...
    struct kc_pending_send *cancel_send_head = ch->token_send_head;
    struct kc_pending_recv *cancel_recv_head = ch->token_recv_head;
    ch->token_send_head = ch->token_send_tail = NULL;
    ch->token_recv_head = ch->token_recv_tail = NULL;
...
again_send_ptr:
    ...
    if (ch->kind == KC_RENDEZVOUS) {
        ...
        if (pending) {
            ...
            kc_token_kernel_callback(pending->ticket, payload);
            free(pending);
            kc_waiter_token_reset(&send_token);
            return 0;
        }
        if (ch->wq_recv_head == NULL || ch->has_value) {
            ...
            if (timeout_ms < 0) {
                ...
                if (!node) { KC_MUTEX_UNLOCK(&ch->mu); return -ENOMEM; }
                kc_ticket ticket = kc_token_kernel_publish_send(ch, (void*)msg.ptr, msg.len, NULL);
                if (ticket.id == 0) {
                    ch->send_eagain++;
                    KC_MUTEX_UNLOCK(&ch->mu);
                    free(node);
...
again_recv_ptr:
    ...
    if (ch->kind == KC_RENDEZVOUS) {
        for (;;) {
            struct kc_pending_send *pending = pending_send_pop(ch);
            if (pending) {
                struct kc_chan_ptrmsg tmp = pending->msg;
                ch->rv_matches++;
