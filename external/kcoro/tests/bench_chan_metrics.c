...
static void usage(const char *prog) {
    fprintf(stderr,
            ...
            "[-c consumers] [-n packets_per_cycle] [-s packet_size_bytes]\n",
            prog);
}
...
int main(int argc, char **argv) {
    ...
    int opt;
    while ((opt = getopt(argc, argv, "d:i:p:c:n:s:h")) != -1) {
        switch (opt) {
    ...
    while (1) {
        ...
        printf("interval %.3fs: packets/s %.2f, Gbps %.3f\n",
    ...
    if (samples > 0) {
        printf("AVERAGE packets/s %.2f, Gbps %.3f\n",
               total_packets / samples, total_gbps / samples);
    }
