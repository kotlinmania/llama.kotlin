...
/* Helper to park coroutine and track it in scheduler's parked counter */
#define KC_CHAN_PARK() do { \
    kc_sched_t *__s = kc_sched_current(); \
    if (__s) kc_sched_mark_parked_in_channel(__s); \
    kcoro_park(); \
    if (__s) kc_sched_mark_unparked_from_channel(__s); \
} while(0)
