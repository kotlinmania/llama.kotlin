...
object Float128Math {
    ...
    val ZERO_BITS: LimbUInt128 = LimbUInt128.zero()
    val ONE_BITS: LimbUInt128 = pack(0, EXP_BIAS, IntArray(7) { 0 })
    val NAN_BITS: LimbUInt128 = pack(0, EXP_MAX, mantissaWithBit(0x8000_0000_0000_0000uL))
    val INF_BITS: LimbUInt128 = pack(0, EXP_MAX, IntArray(7) { 0 })
    val NEG_INF_BITS: LimbUInt128 = pack(1, EXP_MAX, IntArray(7) { 0 })

    // Conversion helpers -----------------------------------------------------------------------

    fun fromFloat64Bits(bits: Long): LimbUInt128 {
    ...
    fun toFloat64Bits(bits: LimbUInt128): Long {
    ...
    fun addBits(a: LimbUInt128, b: LimbUInt128): LimbUInt128 {
    ...
    fun subBits(a: LimbUInt128, b: LimbUInt128): LimbUInt128 = addBits(a, negateBits(b))

    fun mulBits(a: LimbUInt128, b: LimbUInt128): LimbUInt128 {
        if (isNaN(a) || isNaN(b)) return NAN_BITS
    ...
    fun divBits(a: LimbUInt128, b: LimbUInt128): LimbUInt128 {
        if (isNaN(a) || isNaN(b)) return NAN_BITS
        if (isZero(b)) {
            return when {
                isZero(a) -> NAN_BITS
    ...
    internal fun pack(sign: Int, exp: Int, mantissa: IntArray): LimbUInt128 {
    ...
    internal fun unpack(bits: LimbUInt128): Triple<Int, Int, IntArray> {
        val limbs = bits.toIntArray()
    ...
    private fun buildMantissa(source: Long, lsbShift: Int): IntArray {
        require(lsbShift >= 0)
        var base = LimbUInt128.one().shiftLeft(lsbShift)
        var result = LimbUInt128.zero()
        var value = source
        var index = 0
        while (value != 0L) {
            if ((value and 1L) != 0L) {
    ...
    private fun extractMantissa(mantissa: IntArray, shift: Int): Long {
        var value = LimbUInt128.fromLimbsUnsafe(
    ...
    private fun mantissaWithBit(bit: ULong): IntArray {
        var value = LimbUInt128.zero()
        var base = LimbUInt128.one()
        var remaining = bit
        var position = 0
        while (remaining != 0uL) {
