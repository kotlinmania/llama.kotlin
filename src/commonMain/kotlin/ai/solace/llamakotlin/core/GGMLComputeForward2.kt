...
private fun writeFloatRow2(data: ByteArray, off: Int, src: FloatArray, srcOff: Int, n: Int) {
    for (i in 0 until n) {
        writeFloat2(data, off + i * 4, src[srcOff + i])
    }
}
...
private fun binaryOpLocals2(dst: GGMLTensor): BinaryOpLocals2 {
    val src0 = dst.src[0]!!
    val src1 = dst.src[1]!!
    return BinaryOpLocals2(
...
private fun unaryOpLocals2(dst: GGMLTensor): UnaryOpLocals2 {
    val src0 = dst.src[0]!!
    return UnaryOpLocals2(
        ne00 = src0.ne[0], ne01 = src0.ne[1], ne02 = src0.ne[2], ne03 = src0.ne[3],
        nb00 = src0.nb[0].toLong(), nb01 = src0.nb[1].toLong(), nb02 = src0.nb[2].toLong(), nb03 = src0.nb[3].toLong(),
        ne0 = dst.ne[0], ne1 = dst.ne[1], ne2 = dst.ne[2], ne3 = dst.ne[3],
        nb0 = dst.nb[0].toLong(), nb1 = dst.nb[1].toLong(), nb2 = dst.nb[2].toLong(), nb3 = dst.nb[3].toLong()
...
private fun ggml_compute_forward_out_prod_f32(
    ...
    val blck0 = max(GGML_VEC_MAD_UNROLL.toLong(), 32L)
    val blck1 = 16L
        ...
        while (bi01 < l.ne01) {
            ...
            while (ir < bir1) {
                ...
                while (i01 < bne01) {
                    ...
                    val s1Val = readFloat2(src1Data, s1Off)
                    for (i0 in 0 until l.ne0.toInt()) {
                        val s0Val = readFloat2(src0Data, s0Off + i0 * 4)
                        val cur = readFloat2(dstData, dOff + i0 * 4)
                        writeFloat2(dstData, dOff + i0 * 4, cur + s0Val * s1Val)
                    }
                    i01++
                }
                ir++
...
private fun ggml_compute_forward_out_prod_q_f32(
    ...
    require(l.nb00 == ggmlTypeSize(type).toLong())
    ...
    val wdata = FloatArray((l.ne0.toInt() + CACHE_LINE_SIZE_F32_2))
    ...
    for (ir in ir0 until ir1) {
        ...
        for (i01 in 0 until l.ne01) {
            ...
            dequantize_row_2(type, src0Data, s0Off, wdata, 0, l.ne0)
            val s1Val = readFloat2(src1Data, s1Off)
            // ggml_vec_mad_f32(ne0, d, wdata, *s1)
            for (i0 in 0 until l.ne0.toInt()) {
                val cur = readFloat2(dstData, dOff + i0 * 4)
                writeFloat2(dstData, dOff + i0 * 4, cur + wdata[i0] * s1Val)
            }
        }
    }
}
...
private fun ggml_compute_forward_scale_f32(
    ...
    if (b == 0.0f) {
        for (i1 in ir0 until ir1) {
            ...
            val dstOff = (i1 * nb1).toInt()
            val row = readFloatRow2(dstData, dstOff, nc)
            ggml_vec_scale_f32_2(nc.toLong(), row, 0, s)
            writeFloatRow2(dstData, dstOff, row, 0, nc)
        }
    } else {
        for (i1 in ir0 until ir1) {
            val dstOff = (i1 * nb1).toInt()
            val src0Off = (i1 * nb1).toInt()
            val dstRow = readFloatRow2(dstData, dstOff, nc)
            val src0Row = readFloatRow2(src0Data, src0Off, nc)
            ggml_vec_mad1_f32_2(nc.toLong(), dstRow, 0, src0Row, 0, s, b)
            writeFloatRow2(dstData, dstOff, dstRow, 0, nc)
        }
    }
}
...
private fun ggml_compute_forward_set_f32(
    ...
    require(offset + im0 * nb0 + im1 * nb1 + im2 * nb2 + im3 * nb3 <= ggmlNbytes(dst).toLong())
    ...
    for (ir in ir0 until ir1) {
        ...
        for (ic in 0 until nc) {
            val v = readFloat2(src1Data, src1Off + ic * 4)
            writeFloat2(dstData, dstOff + ic * 4, v)
        }
    }
}
...
fun ggml_compute_forward_cpy(
    ...
    ggml_compute_forward_dup_2(params, dst)
}
...
private fun ggml_compute_forward_dup_2(
    ...
    if (ggmlIsContiguous(src0) && ggmlIsContiguous(dst) && src0.type == dst.type) {
        val nBytes = ggmlNbytes(dst).toInt()
    ...
    if (src0.type == GGMLType.F32 && dst.type == GGMLType.F32) {
            ...
            for (i2 in 0 until ne2) {
                for (i1 in 0 until ne1) {
                    for (i0 in 0 until ne0) {
                        ...
                        val v = readFloat2(src0Data, srcOff)
                        writeFloat2(dstData, dstOff, v)
                    }
                }
            }
        }
    } else if (src0.type == GGMLType.F16 && dst.type == GGMLType.F32) {
        for (i3 in 0 until ne3) {
            for (i2 in 0 until ne2) {
                for (i1 in 0 until ne1) {
                    for (i0 in 0 until ne0) {
                        val srcOff = (i3 * nb03 + i2 * nb02 + i1 * nb01 + i0 * nb00).toInt()
                        val dstOff = (i3 * nb3 + i2 * nb2 + i1 * nb1 + i0 * nb0).toInt()
                        val bits = readShort2(src0Data, srcOff)
    ...
    } else if (src0.type == GGMLType.F32 && dst.type == GGMLType.F16) {
        for (i3 in 0 until ne3) {
            for (i2 in 0 until ne2) {
                for (i1 in 0 until ne1) {
                    for (i0 in 0 until ne0) {
        ...
        val nBytes = min(ggmlNbytes(dst).toInt(), min(src0Data.size, dstData.size))
...
private fun ggml_compute_forward_get_rows_q(
    ...
    for (i in ir0 until ir1) {
        ...
        val i01 = readInt2(src1Data, i01Off).toLong()
        ...
        dequantize_row_2(type, src0Data, src0Off, wdata, 0, nc)
        writeFloatRow2(dstData, dstOff, wdata, 0, nc.toInt())
    }
}
...
private fun ggml_compute_forward_get_rows_f16(
    ...
    for (i in ir0 until ir1) {
        ...
        for (j in 0 until nc.toInt()) {
            val bits = readShort2(src0Data, src0Off + j * 2)
            val v = ggmlFp16ToFp32(bits.toUShort())
            writeFloat2(dstData, dstOff + j * 4, v)
        }
    }
}
...
private fun ggml_compute_forward_get_rows_bf16(
    ...
    for (i in ir0 until ir1) {
        ...
        for (j in 0 until nc.toInt()) {
            val bits = readShort2(src0Data, src0Off + j * 2)
            val v = bf16ToF32(GGMLBF16(bits.toUShort()))
            writeFloat2(dstData, dstOff + j * 4, v)
        }
    }
}
    ...
    for (i03 in 0 until l.ne03) {
        for (i02 in 0 until l.ne02) {
            for (i in ir0 until ir1) {
                ...
                val i1 = readInt2(src1Data, idx1Off).toLong()
                ...
                quantize_row_2(dst.type, src0Data, src0Off, dstData, dstOff, nc)
            }
        }
    }
}
    ...
    for (i03 in 0 until l.ne03) {
        for (i02 in 0 until l.ne02) {
            for (i in ir0 until ir1) {
                ...
                val idx1Off = (i10 * l.nb10 + i11 * l.nb11 + i12 * l.nb12).toInt()
                val i1 = readLong2(src1Data, idx1Off)
...
fun ggml_compute_forward_set_rows(
    ...
    when (src0.type) {
        GGMLType.F32 -> {
            when (src1.type) {
                GGMLType.I64 -> ggml_compute_forward_set_rows_f32_i64(params, dst)
                GGMLType.I32 -> ggml_compute_forward_set_rows_f32_i32(params, dst)
...
private fun ggml_compute_forward_get_rows_back_f32_f16(
    ...
    val nc = src0.ne[0].toInt()
    val nr = ggmlNelements(src1).toInt()

    require(dst.ne[0].toLong() == nc.toLong())
    require(src0.nb[0].toLong() == 2L) // sizeof(ggml_fp16_t)

    for (i in 0 until nr) {
        val r = readInt2(src1Data, i * 4)

        for (j in 0 until nc) {
            val bits = readShort2(src0Data, (i * src0.nb[1].toInt()) + j * 2)
            val v = ggmlFp16ToFp32(bits.toUShort())
            val dstOff = r * dst.nb[1].toInt() + j * 4
            val cur = readFloat2(dstData, dstOff)
            writeFloat2(dstData, dstOff, cur + v)
        }
    }
}
...
private fun ggml_compute_forward_get_rows_back_f32(
    ...
    for (i in 0 until nr) {
        ...
        for (j in 0 until nc) {
            val dVal = readFloat2(dstData, dstOff + j * 4)
            val sVal = readFloat2(src0Data, src0Off + j * 4)
            writeFloat2(dstData, dstOff + j * 4, dVal + sVal)
        }
    }
}
        ...
        for (i2 in 0 until l.ne2.toInt()) {
            for (i1 in 0 until l.ne1.toInt()) {
                ...
                for (i0 in 0 until i1) {
                    writeFloat2(dstData, dOff + i0 * 4, 0f)
                }
                val sVal = readFloat2(src0Data, sOff + i1 * 4)
                writeFloat2(dstData, dOff + i1 * 4, sVal)
...
fun ggml_compute_forward_diag_mask_inf(
    ...
    when (src0.type) {
        GGMLType.F32 -> {
            ggml_compute_forward_diag_mask_f32(params, dst, Float.NEGATIVE_INFINITY)
        }
        else -> {
            error("fatal error")
        }
...
fun ggml_compute_forward_diag_mask_zero(
    ...
    when (src0.type) {
        GGMLType.F32 -> {
            ggml_compute_forward_diag_mask_f32(params, dst, 0f)
        }
        else -> {
            error("fatal error")
        }
...
private fun ggml_compute_forward_soft_max_f32(
    ...
    val wp = FloatArray((l.ne00 + CACHE_LINE_SIZE_F32_2).toInt())

    val useF16 = src1 != null && src1.type == GGMLType.F16
        ...
        for (i02 in 0 until l.ne02) {
            for (i01 in ith.toLong() until l.ne01 step nth.toLong()) {
                ...
                for (i in 0 until l.ne00.toInt()) {
                    wp[i] = readFloat2(src0Data, spOff + i * 4)
                }
                // ggml_vec_scale_f32(ne00, wp, scale)
                ggml_vec_scale_f32_2(l.ne00, wp, 0, scale)

                if (src1Data != null) {
                    val maskOff = (i11 * nb11 + i12 * nb12 + i13 * nb13).toInt()
                    if (useF16) {
                        for (i in 0 until l.ne00.toInt()) {
                            val bits = readShort2(src1Data, maskOff + i * 2)
                            wp[i] += slope * ggmlFp16ToFp32(bits.toUShort())
                        }
                    } else {
                        for (i in 0 until l.ne00.toInt()) {
                            wp[i] += slope * readFloat2(src1Data, maskOff + i * 4)
                        }
                    }
                }

                var maxVal = Float.NEGATIVE_INFINITY
                maxVal = ggml_vec_max_f32_2(l.ne00, wp, 0)

                // if we have sinks, make a correction as if they were included in the softmax
                if (src2Data != null) {
                    val skVal = readFloat2(src2Data, i02.toInt() * 4)
                    maxVal = max(maxVal, skVal)
                }

                val dp = FloatArray(l.ne00.toInt())
                var sum = ggml_vec_soft_max_f32_2(l.ne00, dp, 0, wp, 0, maxVal)
                require(sum > 0.0)
                ...
                ggml_vec_scale_f32_2(l.ne00, dp, 0, invSum)

                // write dp to dst
                writeFloatRow2(dstData, dpOff, dp, 0, l.ne00.toInt())
            }
        }
    }
}
...
private fun ggml_compute_forward_soft_max_ext_back_f32(
    ...
    for (i1 in ir0 until ir1) {
        ...
        val dy = readFloatRow2(src0Data, dyOff, nc)
        val y = readFloatRow2(src1Data, yOff, nc)
        val dx = FloatArray(nc)

        // dot_y_dy := dot(y, dy)
        val dotYDy = ggml_vec_dot_f32_2(nc.toLong(), y, 0, dy, 0)
        // dx := dy
        ggml_vec_cpy_f32_2(nc.toLong(), dx, 0, dy, 0)
        // dx := dx - dot_y_dy
        ggml_vec_acc1_f32_2(nc.toLong(), dx, 0, -dotYDy)
        // dx := dx * y
        ggml_vec_mul_f32_2(nc.toLong(), dx, 0, dx, 0, y, 0)
        // dx := dx * scale
        ggml_vec_scale_f32_2(nc.toLong(), dx, 0, scale)

        writeFloatRow2(dstData, dxOff, dx, 0, nc)
    }
}
...
private fun dequantize_row_2(type: GGMLType, src: ByteArray, srcOff: Int, dst: FloatArray, dstOff: Int, k: Long) {
    when (type) {
...
private fun quantize_row_2(type: GGMLType, src: ByteArray, srcOff: Int, dst: ByteArray, dstOff: Int, k: Long) {
    // Read floats from src ByteArray, quantize, write to dst ByteArray
    val floats = readFloatRow2(src, srcOff, k.toInt())
    val tmpDst = ByteArray(ggmlRowSize(type, k).toInt())
    when (type) {
        GGMLType.Q1_0  -> quantize_row_q1_0(floats, tmpDst, k)
        GGMLType.Q4_0  -> quantize_row_q4_0(floats, tmpDst, k)
        GGMLType.Q4_1  -> quantize_row_q4_1(floats, tmpDst, k)
        GGMLType.Q5_0  -> quantize_row_q5_0(floats, tmpDst, k)
        GGMLType.Q5_1  -> quantize_row_q5_1(floats, tmpDst, k)
        GGMLType.Q8_0  -> quantize_row_q8_0_generic(floats, tmpDst, k)
        GGMLType.Q2_K  -> quantize_row_q2_K(floats, tmpDst, k)
        GGMLType.Q3_K  -> quantize_row_q3_K(floats, tmpDst, k)
        GGMLType.Q4_K  -> quantize_row_q4_K(floats, tmpDst, k)
        GGMLType.Q5_K  -> quantize_row_q5_K(floats, tmpDst, k)
        GGMLType.Q6_K  -> quantize_row_q6_K(floats, tmpDst, k)
        GGMLType.TQ1_0 -> quantize_row_tq1_0(floats, tmpDst, k)
        GGMLType.TQ2_0 -> quantize_row_tq2_0(floats, tmpDst, k)
        GGMLType.MXFP4 -> quantize_row_mxfp4(floats, tmpDst, k)
        GGMLType.NVFP4 -> quantize_row_nvfp4(floats, tmpDst, k)
        GGMLType.IQ4_NL -> quantize_row_iq4_nl(floats, tmpDst, k)
        GGMLType.F32 -> {
            // no quantize needed; just copy float bytes
            writeFloatRow2(dst, dstOff, floats, 0, k.toInt())
            return
        }
