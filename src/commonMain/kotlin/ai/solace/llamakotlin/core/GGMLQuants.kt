...
fun quantizeRowQ8_0(x: FloatArray, y: Array<BlockQ8_0>, k: Long) {
    TODO("port from ggml/src/ggml-cpu/quants.c — delegates to quantize_row_q8_0_ref")
}
...
fun quantizeRowQ8_1(x: FloatArray, y: Array<BlockQ8_1>, k: Long) {
    TODO("port from ggml/src/ggml-cpu/quants.c — delegates to quantize_row_q8_1_ref")
}

/** Quantize [k] floats from [x] into MXFP4 blocks in [y]. */
fun quantizeRowMXFP4(x: FloatArray, y: Array<BlockMXFP4>, k: Long) {
    TODO("port from ggml/src/ggml-cpu/quants.c — delegates to quantize_row_mxfp4_ref")
}

/** Quantize [k] floats from [x] into NVFP4 blocks in [y]. */
fun quantizeRowNVFP4(x: FloatArray, y: Array<BlockNVFP4>, k: Long) {
    TODO("port from ggml/src/ggml-cpu/quants.c — delegates to quantize_row_nvfp4_ref")
}

// ── K-quant quantize-row stubs ──────────────────────────────────────────────────

fun quantizeRowQ2K(x: FloatArray, y: Array<BlockQ2K>, k: Long) {
    TODO("port from ggml/src/ggml-cpu/quants.c — delegates to quantize_row_q2_K_ref")
}

fun quantizeRowQ3K(x: FloatArray, y: Array<BlockQ3K>, k: Long) {
    TODO("port from ggml/src/ggml-cpu/quants.c — delegates to quantize_row_q3_K_ref")
}

fun quantizeRowQ4K(x: FloatArray, y: Array<BlockQ4K>, k: Long) {
    require(k % QK_K == 0L) { "k ($k) must be a multiple of QK_K ($QK_K)" }
    TODO("port from ggml/src/ggml-cpu/quants.c — delegates to quantize_row_q4_K_ref")
}

fun quantizeRowQ5K(x: FloatArray, y: Array<BlockQ5K>, k: Long) {
    require(k % QK_K == 0L) { "k ($k) must be a multiple of QK_K ($QK_K)" }
    TODO("port from ggml/src/ggml-cpu/quants.c — delegates to quantize_row_q5_K_ref")
}

fun quantizeRowQ6K(x: FloatArray, y: Array<BlockQ6K>, k: Long) {
    require(k % QK_K == 0L) { "k ($k) must be a multiple of QK_K ($QK_K)" }
    TODO("port from ggml/src/ggml-cpu/quants.c — delegates to quantize_row_q6_K_ref")
}

fun quantizeRowQ8K(x: FloatArray, y: Array<BlockQ8K>, k: Long) {
    TODO("port from ggml/src/ggml-cpu/quants.c — delegates to quantize_row_q8_K_ref")
}
...
fun quantizeRowTQ1_0(x: FloatArray, y: Array<BlockTQ1_0>, k: Long) {
    require(k % QK_K == 0L) { "k ($k) must be a multiple of QK_K ($QK_K)" }
    TODO("port from ggml/src/ggml-cpu/quants.c — delegates to quantize_row_tq1_0_ref")
}

fun quantizeRowTQ2_0(x: FloatArray, y: Array<BlockTQ2_0>, k: Long) {
    require(k % QK_K == 0L) { "k ($k) must be a multiple of QK_K ($QK_K)" }
    TODO("port from ggml/src/ggml-cpu/quants.c — delegates to quantize_row_tq2_0_ref")
}
...
fun quantizeRowIQ4NL(x: FloatArray, y: Array<BlockIQ4NL>, k: Long) {
    ...
    TODO("port from ggml/src/ggml-cpu/quants.c — delegates to quantize_row_iq4_nl_ref")
}

fun quantizeRowIQ4XS(x: FloatArray, y: Array<BlockIQ4XS>, k: Long) {
    require(k % QK_K == 0L) { "k ($k) must be a multiple of QK_K ($QK_K)" }
    TODO("port from ggml/src/ggml-cpu/quants.c — delegates to quantize_iq4_xs")
}
...
fun vecDotQ3KQ8K(n: Int, vx: Array<BlockQ3K>, vy: Array<BlockQ8K>): Float {
    ...
    TODO("port from ggml/src/ggml-cpu/quants.c — ggml_vec_dot_q3_K_q8_K_generic (complex scale unpacking)")
}

/** Q4_K × Q8_K dot product (scalar). */
fun vecDotQ4KQ8K(n: Int, vx: Array<BlockQ4K>, vy: Array<BlockQ8K>): Float {
    require(n % QK_K == 0) { "n ($n) must be divisible by $QK_K" }
    TODO("port from ggml/src/ggml-cpu/quants.c — ggml_vec_dot_q4_K_q8_K_generic (complex scale unpacking)")
}

/** Q5_K × Q8_K dot product (scalar). */
fun vecDotQ5KQ8K(n: Int, vx: Array<BlockQ5K>, vy: Array<BlockQ8K>): Float {
    require(n % QK_K == 0) { "n ($n) must be divisible by $QK_K" }
    TODO("port from ggml/src/ggml-cpu/quants.c — ggml_vec_dot_q5_K_q8_K_generic (complex scale unpacking)")
}

/** Q6_K × Q8_K dot product (scalar). */
fun vecDotQ6KQ8K(n: Int, vx: Array<BlockQ6K>, vy: Array<BlockQ8K>): Float {
    require(n % QK_K == 0) { "n ($n) must be divisible by $QK_K" }
    TODO("port from ggml/src/ggml-cpu/quants.c — ggml_vec_dot_q6_K_q8_K_generic (complex 6-bit unpack)")
}
...
fun vecDotIQ2XXSQ8K(n: Int, vx: Array<BlockIQ2XXS>, vy: Array<BlockQ8K>): Float {
    require(n % QK_K == 0) { "n ($n) must be divisible by $QK_K" }
    TODO("port from ggml/src/ggml-cpu/quants.c — ggml_vec_dot_iq2_xxs_q8_K_generic")
}

fun vecDotIQ2XSQ8K(n: Int, vx: Array<BlockIQ2XS>, vy: Array<BlockQ8K>): Float {
    require(n % QK_K == 0) { "n ($n) must be divisible by $QK_K" }
    TODO("port from ggml/src/ggml-cpu/quants.c — ggml_vec_dot_iq2_xs_q8_K_generic")
}

fun vecDotIQ2SQ8K(n: Int, vx: Array<BlockIQ2S>, vy: Array<BlockQ8K>): Float {
    require(n % QK_K == 0) { "n ($n) must be divisible by $QK_K" }
    TODO("port from ggml/src/ggml-cpu/quants.c — ggml_vec_dot_iq2_s_q8_K_generic")
}

fun vecDotIQ3XXSQ8K(n: Int, vx: Array<BlockIQ3XXS>, vy: Array<BlockQ8K>): Float {
    require(n % QK_K == 0) { "n ($n) must be divisible by $QK_K" }
    TODO("port from ggml/src/ggml-cpu/quants.c — ggml_vec_dot_iq3_xxs_q8_K_generic")
}

fun vecDotIQ3SQ8K(n: Int, vx: Array<BlockIQ3S>, vy: Array<BlockQ8K>): Float {
    require(n % QK_K == 0) { "n ($n) must be divisible by $QK_K" }
    TODO("port from ggml/src/ggml-cpu/quants.c — ggml_vec_dot_iq3_s_q8_K_generic")
}

fun vecDotIQ1SQ8K(n: Int, vx: Array<BlockIQ1S>, vy: Array<BlockQ8K>): Float {
    require(n % QK_K == 0) { "n ($n) must be divisible by $QK_K" }
    TODO("port from ggml/src/ggml-cpu/quants.c — ggml_vec_dot_iq1_s_q8_K_generic")
}

fun vecDotIQ1MQ8K(n: Int, vx: Array<BlockIQ1M>, vy: Array<BlockQ8K>): Float {
    require(n % QK_K == 0) { "n ($n) must be divisible by $QK_K" }
    TODO("port from ggml/src/ggml-cpu/quants.c — ggml_vec_dot_iq1_m_q8_K_generic")
}
...
fun vecDotIQ4XSQ8K(n: Int, vx: Array<BlockIQ4XS>, vy: Array<BlockQ8K>): Float {
    require(n % QK_K == 0) { "n ($n) must be divisible by $QK_K" }
    TODO("port from ggml/src/ggml-cpu/quants.c — ggml_vec_dot_iq4_xs_q8_K_generic")
}
