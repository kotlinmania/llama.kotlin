 ...
 * | C name            | Kotlin name      |
 * |--------------------|-----------------|
 * | `block_q4_0`       | [BlockQ4_0]     |
 * | `block_q2_K`       | [BlockQ2K]      |
 * | `block_iq2_xxs`    | [BlockIQ2XXS]   |
 * | `iq1m_scale_t`     | [IQ1MScale]     |
 */

// ── Re-exported constants already defined in GGMLTypes.kt ──────────────────────
// Callers can `import ai.solace.llamakotlin.core.*` and get everything.
...
internal const val IQ3S_N_SCALE: Int = QK_K / 64
...
internal const val QI1_0: Int = QK1_0 / 32

internal const val QR4_0: Int = 2
internal const val QI4_0: Int = QK4_0 / (4 * QR4_0)

internal const val QR4_1: Int = 2
internal const val QI4_1: Int = QK4_1 / (4 * QR4_1)

internal const val QR_MXFP4: Int = 2
internal const val QI_MXFP4: Int = QK_MXFP4 / (4 * QR_MXFP4)

internal const val QR_NVFP4: Int = 2
internal const val QI_NVFP4: Int = QK_NVFP4 / (4 * QR_NVFP4)

internal const val QR5_0: Int = 2
internal const val QI5_0: Int = QK5_0 / (4 * QR5_0)

internal const val QR5_1: Int = 2
internal const val QI5_1: Int = QK5_1 / (4 * QR5_1)

internal const val QR8_0: Int = 1
internal const val QI8_0: Int = QK8_0 / (4 * QR8_0)

internal const val QR8_1: Int = 1
internal const val QI8_1: Int = QK8_1 / (4 * QR8_1)

internal const val QR2_K: Int = 4
internal const val QI2_K: Int = QK_K / (4 * QR2_K)

internal const val QR3_K: Int = 4
internal const val QI3_K: Int = QK_K / (4 * QR3_K)

internal const val QR4_K: Int = 2
internal const val QI4_K: Int = QK_K / (4 * QR4_K)

internal const val QR5_K: Int = 2
internal const val QI5_K: Int = QK_K / (4 * QR5_K)

internal const val QR6_K: Int = 2
internal const val QI6_K: Int = QK_K / (4 * QR6_K)

internal const val QR2_XXS: Int = 4
internal const val QI2_XXS: Int = QK_K / (4 * QR2_XXS)

internal const val QR2_XS: Int = 4
internal const val QI2_XS: Int = QK_K / (4 * QR2_XS)

internal const val QR2_S: Int = 4
internal const val QI2_S: Int = QK_K / (4 * QR2_S)

internal const val QR3_XXS: Int = 4
internal const val QI3_XXS: Int = QK_K / (4 * QR3_XXS)

internal const val QR3_XS: Int = 4
internal const val QI3_XS: Int = QK_K / (4 * QR3_XS)

internal const val QR1_S: Int = 8
internal const val QI1_S: Int = QK_K / (4 * QR1_S)

internal const val QR1_M: Int = 8
internal const val QI1_M: Int = QK_K / (4 * QR1_M)

internal const val QR4_NL: Int = 2
internal const val QI4_NL: Int = QK4_NL / (4 * QR4_NL)

internal const val QR4_XS: Int = 2
internal const val QI4_XS: Int = QK_K / (4 * QR4_XS)

internal const val QR3_S: Int = 4
internal const val QI3_S: Int = QK_K / (4 * QR3_S)
 ...
 * @property qs    bit-packed quants — 1 bit per element, [QK1_0] / 8 = 16 bytes
 */
data class BlockQ1_0(
    val d: Short,
    val qs: ByteArray = ByteArray(QK1_0 / 8),
) {
    init {
        require(qs.size == QK1_0 / 8) { "qs.size must be ${QK1_0 / 8}" }
    }
    ...
    companion object {
        ...
        const val SIZE_BYTES: Int = 2 + QK1_0 / 8
    }
}
 ...
 *
 * @property d     delta (scale), stored as raw fp16 bits
 * @property qs    nibble-packed quants — 2 elements per byte, [QK4_0] / 2 = 16 bytes
 */
data class BlockQ4_0(
    val d: Short,
    val qs: ByteArray = ByteArray(QK4_0 / 2),
) {
    init {
        require(qs.size == QK4_0 / 2) { "qs.size must be ${QK4_0 / 2}" }
    }
    ...
    companion object {
        ...
        const val SIZE_BYTES: Int = 2 + QK4_0 / 2
    }
}
...
data class BlockQ4_1(
    ...
    companion object {
        ...
        const val SIZE_BYTES: Int = 2 * 2 + QK4_1 / 2
    }
}
 ...
 * @property qs    packed 4-bit FP values, [QK_MXFP4] / 2 = 16 bytes
 */
data class BlockMXFP4(
    val e: Byte,
    val qs: ByteArray = ByteArray(QK_MXFP4 / 2),
) {
    init {
        require(qs.size == QK_MXFP4 / 2) { "qs.size must be ${QK_MXFP4 / 2}" }
    }
    ...
    companion object {
        ...
        const val SIZE_BYTES: Int = 1 + QK_MXFP4 / 2
    }
}
 ...
 * @property qs    packed 4-bit E2M1 values, [QK_NVFP4] / 2 = 32 bytes
 */
data class BlockNVFP4(
    val d: ByteArray = ByteArray(QK_NVFP4 / QK_NVFP4_SUB),
    val qs: ByteArray = ByteArray(QK_NVFP4 / 2),
) {
    init {
        require(d.size == QK_NVFP4 / QK_NVFP4_SUB) { "d.size must be ${QK_NVFP4 / QK_NVFP4_SUB}" }
        require(qs.size == QK_NVFP4 / 2) { "qs.size must be ${QK_NVFP4 / 2}" }
    }
    ...
    companion object {
        ...
        const val SIZE_BYTES: Int = QK_NVFP4 / QK_NVFP4_SUB + QK_NVFP4 / 2
    }
}
 ...
 * @property qs    lower 4 bits of quants, nibble-packed, [QK5_0] / 2 = 16 bytes
 */
data class BlockQ5_0(
    val d: Short,
    val qh: ByteArray = ByteArray(4),
    val qs: ByteArray = ByteArray(QK5_0 / 2),
) {
    init {
        require(qh.size == 4) { "qh.size must be 4" }
        require(qs.size == QK5_0 / 2) { "qs.size must be ${QK5_0 / 2}" }
    }
    ...
    companion object {
        ...
        const val SIZE_BYTES: Int = 2 + 4 + QK5_0 / 2
    }
}
 ...
 * @property qs    lower 4 bits of quants, nibble-packed, [QK5_1] / 2 = 16 bytes
 */
data class BlockQ5_1(
    val d: Short,
    val m: Short,
    val qh: ByteArray = ByteArray(4),
    val qs: ByteArray = ByteArray(QK5_1 / 2),
) {
    val dm: Int get() = (d.toInt() and 0xFFFF) or ((m.toInt() and 0xFFFF) shl 16)

    init {
        require(qh.size == 4) { "qh.size must be 4" }
        require(qs.size == QK5_1 / 2) { "qs.size must be ${QK5_1 / 2}" }
    }
    ...
    companion object {
        ...
        const val SIZE_BYTES: Int = 2 * 2 + 4 + QK5_1 / 2
    }
}
 ...
 *
 * Layout (C): `ggml_half d; int8_t qs[QK8_0];`
 *
 * @property d     delta (scale), stored as raw fp16 bits
 * @property qs    signed 8-bit quants, [QK8_0] = 32 bytes
 */
data class BlockQ8_0(
    val d: Short,
    val qs: ByteArray = ByteArray(QK8_0),
) {
    init {
        require(qs.size == QK8_0) { "qs.size must be $QK8_0" }
    }
    ...
    companion object {
        ...
        const val SIZE_BYTES: Int = 2 + QK8_0
    }
}
 ...
 * @property qs    signed 8-bit quants, [QK8_1] = 32 bytes
 */
data class BlockQ8_1(
    val d: Short,
    val s: Short,
    val qs: ByteArray = ByteArray(QK8_1),
) {
    val ds: Int get() = (d.toInt() and 0xFFFF) or ((s.toInt() and 0xFFFF) shl 16)

    init {
        require(qs.size == QK8_1) { "qs.size must be $QK8_1" }
    }
    ...
    companion object {
        ...
        const val SIZE_BYTES: Int = 2 * 2 + QK8_1
    }
}
...
data class BlockTQ1_0(
    val qs: ByteArray = ByteArray((QK_K - 4 * QK_K / 64) / 5),
    val qh: ByteArray = ByteArray(QK_K / 64),
    val d: Short,
) {
    init {
        require(qs.size == (QK_K - 4 * QK_K / 64) / 5) { "qs.size must be ${(QK_K - 4 * QK_K / 64) / 5}" }
        require(qh.size == QK_K / 64) { "qh.size must be ${QK_K / 64}" }
    }
    ...
    companion object {
        ...
        const val SIZE_BYTES: Int = 2 + QK_K / 64 + (QK_K - 4 * QK_K / 64) / 5
    }
}
 ...
 *
 * @property qs    2-bit packed quants, [QK_K] / 4 = 64 bytes
 * @property d     scale, stored as raw fp16 bits
 */
data class BlockTQ2_0(
    val qs: ByteArray = ByteArray(QK_K / 4),
    val d: Short,
) {
    init {
        require(qs.size == QK_K / 4) { "qs.size must be ${QK_K / 4}" }
    }
    ...
    companion object {
        ...
        const val SIZE_BYTES: Int = 2 + QK_K / 4
    }
}
 ...
 *
 * @property scales  scales and mins, quantized with 4 bits, [QK_K]/16 = 16 bytes
 * @property qs      2-bit packed quants, [QK_K]/4 = 64 bytes
 ...
 */
data class BlockQ2K(
    val scales: ByteArray = ByteArray(QK_K / 16),
    val qs: ByteArray = ByteArray(QK_K / 4),
    val d: Short,
    val dmin: Short,
) {
    ...
    init {
        require(scales.size == QK_K / 16) { "scales.size must be ${QK_K / 16}" }
        require(qs.size == QK_K / 4) { "qs.size must be ${QK_K / 4}" }
    }
    ...
    companion object {
        ...
        const val SIZE_BYTES: Int = 2 * 2 + QK_K / 16 + QK_K / 4
    }
}
 ...
 *
 * @property hmask   high-bit mask for quants, [QK_K]/8 = 32 bytes
 * @property qs      lower 2 bits of quants, [QK_K]/4 = 64 bytes
 * @property scales  6-bit quantized scales, 12 bytes
 * @property d       super-block scale (raw fp16 bits)
 */
data class BlockQ3K(
    val hmask: ByteArray = ByteArray(QK_K / 8),
    val qs: ByteArray = ByteArray(QK_K / 4),
    val scales: ByteArray = ByteArray(12),
    val d: Short,
) {
    init {
        require(hmask.size == QK_K / 8) { "hmask.size must be ${QK_K / 8}" }
        require(qs.size == QK_K / 4) { "qs.size must be ${QK_K / 4}" }
    ...
    companion object {
        ...
        const val SIZE_BYTES: Int = 2 + QK_K / 4 + QK_K / 8 + 12
    }
}
 ...
 * @property d       super-block scale for quantized scales (raw fp16 bits)
 * @property dmin    super-block scale for quantized mins (raw fp16 bits)
 * @property scales  6-bit quantized scales and mins, [K_SCALE_SIZE] = 12 bytes
 * @property qs      4-bit packed quants, [QK_K]/2 = 128 bytes
 */
data class BlockQ4K(
    val d: Short,
    val dmin: Short,
    val scales: ByteArray = ByteArray(K_SCALE_SIZE),
    val qs: ByteArray = ByteArray(QK_K / 2),
) {
    val dm: Int get() = (d.toInt() and 0xFFFF) or ((dmin.toInt() and 0xFFFF) shl 16)

    init {
        require(scales.size == K_SCALE_SIZE) { "scales.size must be $K_SCALE_SIZE" }
        require(qs.size == QK_K / 2) { "qs.size must be ${QK_K / 2}" }
    }
    ...
    companion object {
        ...
        const val SIZE_BYTES: Int = 2 * 2 + K_SCALE_SIZE + QK_K / 2
    }
}
 ...
 *
 * Layout (C): union { struct { ggml_half d; ggml_half dmin; }; ggml_half2 dm; };
 *             uint8_t scales[K_SCALE_SIZE]; uint8_t qh[QK_K/8]; uint8_t qs[QK_K/2];
 *
 * @property d       super-block scale for quantized scales (raw fp16 bits)
 * @property dmin    super-block scale for quantized mins (raw fp16 bits)
 * @property scales  6-bit quantized scales, [K_SCALE_SIZE] = 12 bytes
 * @property qh      high bits of quants, [QK_K]/8 = 32 bytes
 * @property qs      low 4 bits of quants, [QK_K]/2 = 128 bytes
 */
data class BlockQ5K(
    val d: Short,
    val dmin: Short,
    val scales: ByteArray = ByteArray(K_SCALE_SIZE),
    val qh: ByteArray = ByteArray(QK_K / 8),
    val qs: ByteArray = ByteArray(QK_K / 2),
) {
    val dm: Int get() = (d.toInt() and 0xFFFF) or ((dmin.toInt() and 0xFFFF) shl 16)
    ...
    init {
        require(scales.size == K_SCALE_SIZE) { "scales.size must be $K_SCALE_SIZE" }
        require(qh.size == QK_K / 8) { "qh.size must be ${QK_K / 8}" }
        require(qs.size == QK_K / 2) { "qs.size must be ${QK_K / 2}" }
    }
    ...
    companion object {
        ...
        const val SIZE_BYTES: Int = 2 * 2 + K_SCALE_SIZE + QK_K / 8 + QK_K / 2
    }
}
 ...
 *
 * @property ql      lower 4 bits of quants, [QK_K]/2 = 128 bytes
 * @property qh      upper 2 bits of quants, [QK_K]/4 = 64 bytes
 * @property scales  8-bit quantized scales, [QK_K]/16 = 16 bytes
 * @property d       super-block scale (raw fp16 bits)
 */
data class BlockQ6K(
    val ql: ByteArray = ByteArray(QK_K / 2),
    val qh: ByteArray = ByteArray(QK_K / 4),
    val scales: ByteArray = ByteArray(QK_K / 16),
    val d: Short,
) {
    init {
        require(ql.size == QK_K / 2) { "ql.size must be ${QK_K / 2}" }
        require(qh.size == QK_K / 4) { "qh.size must be ${QK_K / 4}" }
        require(scales.size == QK_K / 16) { "scales.size must be ${QK_K / 16}" }
    }
    ...
    companion object {
        ...
        const val SIZE_BYTES: Int = 2 + QK_K / 16 + 3 * QK_K / 4
    }
}
 ...
 *
 * Layout (C): `float d; int8_t qs[QK_K]; int16_t bsums[QK_K/16];`
 *
 * @property d       delta (scale), stored as 32-bit float
 * @property qs      signed 8-bit quants, [QK_K] = 256 bytes
 * @property bsums   sum of quants in groups of 16, [QK_K]/16 = 16 entries (stored as ShortArray)
 */
data class BlockQ8K(
    val d: Float,
    val qs: ByteArray = ByteArray(QK_K),
    val bsums: ShortArray = ShortArray(QK_K / 16),
) {
    init {
        require(qs.size == QK_K) { "qs.size must be $QK_K" }
        require(bsums.size == QK_K / 16) { "bsums.size must be ${QK_K / 16}" }
    }
    ...
    companion object {
        ...
        const val SIZE_BYTES: Int = 4 + QK_K + (QK_K / 16) * 2
    }
}
 ...
 * @property qs    grid indices, [QK_K]/8 = 32 entries stored as ShortArray (uint16_t)
 */
data class BlockIQ2XXS(
    val d: Short,
    val qs: ShortArray = ShortArray(QK_K / 8),
) {
    init {
        require(qs.size == QK_K / 8) { "qs.size must be ${QK_K / 8}" }
    }
    ...
    companion object {
        ...
        const val SIZE_BYTES: Int = 2 + (QK_K / 8) * 2
    }
}
 ...
 *
 * @property d       scale (raw fp16 bits)
 * @property qs      grid indices, [QK_K]/8 = 32 entries (ShortArray)
 * @property scales  block scales, [QK_K]/32 = 8 bytes
 */
data class BlockIQ2XS(
    val d: Short,
    val qs: ShortArray = ShortArray(QK_K / 8),
    val scales: ByteArray = ByteArray(QK_K / 32),
) {
    init {
        require(qs.size == QK_K / 8) { "qs.size must be ${QK_K / 8}" }
        require(scales.size == QK_K / 32) { "scales.size must be ${QK_K / 32}" }
    }
    ...
    companion object {
        ...
        const val SIZE_BYTES: Int = 2 + (QK_K / 8) * 2 + QK_K / 32
    }
}
 ...
 *
 * @property d       scale (raw fp16 bits)
 * @property qs      quants, [QK_K]/4 = 64 bytes
 * @property qh      high bits, [QK_K]/32 = 8 bytes
 * @property scales  block scales, [QK_K]/32 = 8 bytes
 */
data class BlockIQ2S(
    val d: Short,
    val qs: ByteArray = ByteArray(QK_K / 4),
    val qh: ByteArray = ByteArray(QK_K / 32),
    val scales: ByteArray = ByteArray(QK_K / 32),
) {
    init {
        require(qs.size == QK_K / 4) { "qs.size must be ${QK_K / 4}" }
        require(qh.size == QK_K / 32) { "qh.size must be ${QK_K / 32}" }
        require(scales.size == QK_K / 32) { "scales.size must be ${QK_K / 32}" }
    }
    ...
    companion object {
        ...
        const val SIZE_BYTES: Int = 2 + QK_K / 4 + QK_K / 16
    }
}
 ...
 *
 * @property d     scale (raw fp16 bits)
 * @property qs    packed quants, 3 * [QK_K] / 8 = 96 bytes
 */
data class BlockIQ3XXS(
    val d: Short,
    val qs: ByteArray = ByteArray(3 * QK_K / 8),
) {
    init {
        require(qs.size == 3 * QK_K / 8) { "qs.size must be ${3 * QK_K / 8}" }
    }
    ...
    companion object {
        ...
        const val SIZE_BYTES: Int = 2 + 3 * (QK_K / 8)
    }
}
 ...
 * @property qs      quants, [QK_K]/4 = 64 bytes
 * @property qh      high bits, [QK_K]/32 = 8 bytes
 * @property signs   sign bits, [QK_K]/8 = 32 bytes
 * @property scales  block scales, [IQ3S_N_SCALE] = 4 bytes
 */
data class BlockIQ3S(
    val d: Short,
    val qs: ByteArray = ByteArray(QK_K / 4),
    val qh: ByteArray = ByteArray(QK_K / 32),
    val signs: ByteArray = ByteArray(QK_K / 8),
    val scales: ByteArray = ByteArray(IQ3S_N_SCALE),
) {
    init {
        require(qs.size == QK_K / 4) { "qs.size must be ${QK_K / 4}" }
        require(qh.size == QK_K / 32) { "qh.size must be ${QK_K / 32}" }
        require(signs.size == QK_K / 8) { "signs.size must be ${QK_K / 8}" }
        require(scales.size == IQ3S_N_SCALE) { "scales.size must be $IQ3S_N_SCALE" }
    }
    ...
    companion object {
        ...
        const val SIZE_BYTES: Int = 2 + 13 * (QK_K / 32) + IQ3S_N_SCALE
    }
}
 ...
 *
 * @property d     scale (raw fp16 bits)
 * @property qs    grid indices (low 8 bits), [QK_K]/8 = 32 bytes
 * @property qh    grid indices (high bits + sign), [QK_K]/32 = 8 entries (ShortArray)
 */
data class BlockIQ1S(
    val d: Short,
    val qs: ByteArray = ByteArray(QK_K / 8),
    val qh: ShortArray = ShortArray(QK_K / 32),
) {
    init {
        require(qs.size == QK_K / 8) { "qs.size must be ${QK_K / 8}" }
        require(qh.size == QK_K / 32) { "qh.size must be ${QK_K / 32}" }
    ...
    companion object {
        ...
        const val SIZE_BYTES: Int = 2 + QK_K / 8 + QK_K / 16
    }
}
 ...
 * using the [IQ1MScale] union.
 *
 * @property qs      grid index, low 8 bits, [QK_K]/8 = 32 bytes
 * @property qh      grid index, high 3 bits + shift, [QK_K]/16 = 16 bytes
 * @property scales  3-bit block scales, [QK_K]/32 = 8 bytes
 */
data class BlockIQ1M(
    val qs: ByteArray = ByteArray(QK_K / 8),
    val qh: ByteArray = ByteArray(QK_K / 16),
    val scales: ByteArray = ByteArray(QK_K / 32),
) {
    init {
        require(qs.size == QK_K / 8) { "qs.size must be ${QK_K / 8}" }
        require(qh.size == QK_K / 16) { "qh.size must be ${QK_K / 16}" }
        require(scales.size == QK_K / 32) { "scales.size must be ${QK_K / 32}" }
    }
    ...
    companion object {
        ...
        const val SIZE_BYTES: Int = QK_K / 8 + QK_K / 16 + QK_K / 32
    }
}
 ...
 *
 * @property d     scale (raw fp16 bits)
 * @property qs    nibble-packed quants, [QK4_NL]/2 = 16 bytes
 */
data class BlockIQ4NL(
    val d: Short,
    val qs: ByteArray = ByteArray(QK4_NL / 2),
) {
    init {
        require(qs.size == QK4_NL / 2) { "qs.size must be ${QK4_NL / 2}" }
    }
    ...
    companion object {
        ...
        const val SIZE_BYTES: Int = 2 + QK4_NL / 2
    }
}
 ...
 * @property scalesL  low bits of sub-block scales, [QK_K]/64 = 4 bytes
 * @property qs       nibble-packed quants, [QK_K]/2 = 128 bytes
 */
data class BlockIQ4XS(
    val d: Short,
    val scalesH: Short,
    val scalesL: ByteArray = ByteArray(QK_K / 64),
    val qs: ByteArray = ByteArray(QK_K / 2),
) {
    init {
        require(scalesL.size == QK_K / 64) { "scalesL.size must be ${QK_K / 64}" }
        require(qs.size == QK_K / 2) { "qs.size must be ${QK_K / 2}" }
    }
    ...
    companion object {
        ...
        const val SIZE_BYTES: Int = 2 + 2 + QK_K / 64 + QK_K / 2
    }
}
...
internal fun verifyBlockSizes() {
    check(BlockQ1_0.SIZE_BYTES == 2 + QK1_0 / 8) { "wrong q1_0 block size" }
    check(BlockQ4_0.SIZE_BYTES == 2 + QK4_0 / 2) { "wrong q4_0 block size" }
    check(BlockQ4_1.SIZE_BYTES == 2 * 2 + QK4_1 / 2) { "wrong q4_1 block size" }
    check(BlockMXFP4.SIZE_BYTES == 1 + QK_MXFP4 / 2) { "wrong mxfp4 block size" }
    check(BlockNVFP4.SIZE_BYTES == QK_NVFP4 / QK_NVFP4_SUB + QK_NVFP4 / 2) { "wrong nvfp4 block size" }
    check(BlockQ5_0.SIZE_BYTES == 2 + 4 + QK5_0 / 2) { "wrong q5_0 block size" }
    check(BlockQ5_1.SIZE_BYTES == 2 * 2 + 4 + QK5_1 / 2) { "wrong q5_1 block size" }
    check(BlockQ8_0.SIZE_BYTES == 2 + QK8_0) { "wrong q8_0 block size" }
    check(BlockQ8_1.SIZE_BYTES == 2 * 2 + QK8_1) { "wrong q8_1 block size" }
    check(BlockTQ1_0.SIZE_BYTES == 2 + QK_K / 64 + (QK_K - 4 * QK_K / 64) / 5) { "wrong tq1_0 block size" }
    check(BlockTQ2_0.SIZE_BYTES == 2 + QK_K / 4) { "wrong tq2_0 block size" }
    check(BlockQ2K.SIZE_BYTES == 2 * 2 + QK_K / 16 + QK_K / 4) { "wrong q2_K block size" }
    check(BlockQ3K.SIZE_BYTES == 2 + QK_K / 4 + QK_K / 8 + 12) { "wrong q3_K block size" }
    check(BlockQ4K.SIZE_BYTES == 2 * 2 + K_SCALE_SIZE + QK_K / 2) { "wrong q4_K block size" }
    check(BlockQ5K.SIZE_BYTES == 2 * 2 + K_SCALE_SIZE + QK_K / 8 + QK_K / 2) { "wrong q5_K block size" }
    check(BlockQ6K.SIZE_BYTES == 2 + QK_K / 16 + 3 * QK_K / 4) { "wrong q6_K block size" }
    check(BlockQ8K.SIZE_BYTES == 4 + QK_K + (QK_K / 16) * 2) { "wrong q8_K block size" }
    check(BlockIQ2XXS.SIZE_BYTES == 2 + (QK_K / 8) * 2) { "wrong iq2_xxs block size" }
    check(BlockIQ2XS.SIZE_BYTES == 2 + (QK_K / 8) * 2 + QK_K / 32) { "wrong iq2_xs block size" }
    check(BlockIQ2S.SIZE_BYTES == 2 + QK_K / 4 + QK_K / 16) { "wrong iq2_s block size" }
    check(BlockIQ3XXS.SIZE_BYTES == 2 + 3 * (QK_K / 8)) { "wrong iq3_xxs block size" }
    check(BlockIQ3S.SIZE_BYTES == 2 + 13 * (QK_K / 32) + IQ3S_N_SCALE) { "wrong iq3_s block size" }
    check(BlockIQ1S.SIZE_BYTES == 2 + QK_K / 8 + QK_K / 16) { "wrong iq1_s block size" }
    check(BlockIQ1M.SIZE_BYTES == QK_K / 8 + QK_K / 16 + QK_K / 32) { "wrong iq1_m block size" }
    check(BlockIQ4NL.SIZE_BYTES == 2 + QK4_NL / 2) { "wrong iq4_nl block size" }
    check(BlockIQ4XS.SIZE_BYTES == 2 + 2 + QK_K / 64 + QK_K / 2) { "wrong iq4_xs block size" }
}
...
private val _blockSizeVerification: Unit = verifyBlockSizes()
