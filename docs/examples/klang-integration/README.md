# KLang Integration Examples

Reference implementations from [ember-ml-kotlin](https://github.com/KotlinMania/ember-ml-kotlin) demonstrating best practices for using KLang's heap, pointers, and C types for high-performance tensor storage.

## Files

### commonMain/KlangHeapTensorStorage.kt
Multiplatform tensor storage using KLang's GlobalHeap with aligned allocation. Demonstrates:
...
These files serve as **reference implementations** for migrating llama.kotlin's tensor storage to use KLang's GlobalHeap and CFloat32. Key patterns to adopt:
...
See `docs/kdocs/klang-heap-migration-plan.md` for the full strategy to integrate these patterns into llama.kotlin.
